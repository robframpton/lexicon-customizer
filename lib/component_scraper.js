...
var PATH_LEXICON = path.join(__dirname, '../lexicon');

var PATH_ATLAS_THEME_VARIABLES = path.join(PATH_LEXICON, 'src/scss/atlas-theme/variables');

var PATH_ATLAS_THEME_VARIABLES_FILE = path.join(PATH_LEXICON, 'src/scss/atlas-theme/_variables.scss');
...
var PATH_LEXICON_BASE_VARIABLES_FILE = path.join(PATH_LEXICON, 'src/scss/lexicon-base/_variables.scss');
...
module.exports = {
	...
	getLexiconBaseComponentArray: function() {
	...
	mapLexiconVariables: function() {
		return this.mapVariables(this.getLexiconBaseComponentArray(), PATH_LEXICON_BASE_VARIABLES, 'lexicon-base');
	},

	mapVariables: function(componentArray, variablesPath, namespace) {
		var instance = this;

		var variableHierarchyArray = [];

		var variableMap = _.reduce(componentArray, function(result, item, index) {
			var fileName = '_' + item + '.scss';

			var componentVariables = instance.getVariablesFromFile(path.join(variablesPath, fileName));

			variableHierarchyArray = variableHierarchyArray.concat(_.keys(componentVariables))
			...
			componentObject[item] = componentVariables;
		...
		// return {
		// 	variableHierarchyArray: variableHierarchyArray,
